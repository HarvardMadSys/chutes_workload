...
The two output names are the paper's include paths verbatim, quirks and all:
the minimax stem ends in ``_`` before ``_repro`` and the v32 stem does not,
and both carry ``repro`` because that is the subdirectory name the draft
includes them from. Do not "fix" the asymmetry — the names are the contract
with the LaTeX.

Usage:
  python pipeline/caching/plot_paper_figures.py data/caching/results.txt \
